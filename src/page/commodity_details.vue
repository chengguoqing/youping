...
-->
          ...
          <van-row class="pd fz12 df_kjj_Deet">
              <van-col span="8" class="z6">快递:包邮</van-col>
      ...
      <section class="pd df_jhj_tl z9 bgff fz13 pr" @click="popupVerisible=true"  v-if="three_coupon>0||available_coupon>0"><span class="cz">领 券</span> 
        ...
        <span class="quanh_dsdff fz12" v-for="sd in d_info.three_coupon"><i class="f_p jkhg_ererrt"></i>{{sd}}<i class="f_p jkhg_ererrt ab"></i></span>
          ...
          <i class="f_p gengduo_icon"></i>
            ...
            <section class="pd df_jhj_tl z9 bgff fz13 pr btm" @click="$store.state.is_dizs=true"><span class="cz fl">送 至</span> 
...
<!--    送至地址-->
    <szdz :address="address" @cs_eer="cs_eer"></szdz>
...
<van-goods-action class="fg_jh_btn" v-if="is_jh_sd">
<van-goods-action-mini-btn icon="dx  dx icon-dianpu z9" text="进店"   @click="hf('')"/>
  ...
  <van-goods-action-mini-btn icon="dx dx icon-gouwuche1 z9" text="购物车"  @click="hf('gwc')" :info="$store.state.carnumber"  class="sd_deeert" :class="$store.state.carnumber==0?'act':''"/>
      ...
      <van-goods-action-big-btn class="fz14" v-if="d_info.goods_number>0&&d_info.is_on_sale==1" @click="csjkher(act_id)">
          {{act_id?'参与活动':'加入购物车'}}    
  ...
  <van-goods-action-big-btn text="立即购买"   class="fz14" primary @click="popupVisible=true;jiali=1"  v-if="d_info.goods_number>0&&d_info.is_on_sale==1"/>
      ...
      <van-goods-action-big-btn text="商品已售罄"   class="fz14 xiajiassrx" primary  v-if="d_info.goods_number<=0&&d_info.is_on_sale==1"></van-goods-action-big-btn>
    
          <van-goods-action-big-btn text="商品已下架"   class="fz14 xiajiassrx" primary   v-if="d_info.is_on_sale==0"></van-goods-action-big-btn>
    ...
    export default {
        ...
        mounted() {
            ...
            this.post("product/info", { //获取详情信息
                ...
                th.Title("商品详情")
                setTimeout(function() {
                    let sd_ert = `https://api.cangniaowl.com/shop/${th.$store.state.shopinfo.shop_id}.html?router=commodity_details&goods_id=${goods_id}#/commodity_details?goods_id=${goods_id}`
                    th.fenxiang(data.goods_name, sd_ert, data.goods_thumb, data.goods_name)
                }, 1500);
